...
public class MoveObjectController : MonoBehaviour 
{
	...
	{		
		...
		{			
			playerEntered = false;
			PlayerVoiceProcess.instance.micState = false;
		...
		{	
			...
			if (Physics.Raycast(rayOrigin,fpsCam.transform.forward, out hit,reachRange,rayLayerMask))
			{
				...
				if (moveableObject != null)		//hit object must have MoveableDraw script attached
				{
					...
					if (Input.GetKeyUp(KeyCode.E)|| PlayerVoiceProcess.instance.playerSpeech == (isOpen==false?"Open":"Close"))
					{
						anim.enabled = true;					
						anim.SetBool(animBoolNameNum,!isOpen);
						PlayerVoiceProcess.instance.SoundInteract(!isOpen);
						msg = getGuiMsg(!isOpen);
	...
	private void SetUpGUI()
	{
		...
		guiStyle.font = player.GetComponent<PlayerVoiceProcess>().playerFont;
	...
	void OnGUI()
	{
		...
		{
			GUI.Label(new Rect (Screen.width/2 -80 ,Screen.height/2 -80 ,200,50), msg, guiStyle);
		}
	}		
	//End of GUI Config --------------
	#endregion
